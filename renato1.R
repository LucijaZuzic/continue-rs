...
setwd('E:/Academic/0 Research/000 Published/2024/000 TEC predictive model for short-term rapid geomagnetic storm events_MDPI/')
...
#Data input: TEC and dTEC data derived from RINEX GPS observations taken at Matera, Italy
...
for(j in 76:78){
  ...
  fiu <- -12.8437111 * pi / 90 # latitude of Pore?, 45.22601300
  lou <- 131.1327361 * pi / 90 # longitude of Pore?, 13.59504100
  ...
  #plot(tr, tec, xlab = 'time [days]', ylab = 'equivalent ionospheric delay [m]')
}
...
plot(it, rtec1, type='l', col ='red', ylim=c(0,150),ylab = 'TEC [TECU]', 
     xlab = 'days in 2015', cex.lab=2,cex.axis=2)
lines(it,ktec1$V2, col ='blue')
legend("topright", inset=.005, 
       c("experimental (true) TEC","Klobuchar model TEC"), 
       fill=c('red','blue'), horiz=FALSE, cex=1.4)
...
#im <- paste(dst.frame$V1, dst.frame$V2)
#res <- as.POSIXlt(im, "%d/%m/%y %H:%M:%OS")
...
plot(dst1$time, dst1$Dst, type='l', col='red', xlab='time [days in March, 2015]', ylab='Dst [nT]')


dst.frame <- as.data.frame(read.csv('Dst.May2017.dat', header=TRUE, sep ='', skip=24))
dst.frame <-dst.frame [,-5]
colnames(dst.frame) <- c('V1', 'V2', 'DOY', 'Dst')
res <- as.POSIXlt(paste(dst.frame$V1, dst.frame$V2), "%d/%m/%y %H:%M:%OS")
timer <- res$mday + res$hour/24 + res$min/(24*60) + res$sec/(24*3600)## TO SOLVE day of the month!
timer <- round(timer, digits = 4)
...
plot(dst2$time, dst2$Dst, type='l', col='red', xlab='time [days in May, 2017]', ylab='Dst [nT]')

dst.frame <- as.data.frame(read.csv('Dst.Sep2017.dat', header=TRUE, sep ='', skip=24))
dst.frame <-dst.frame [,-5]
colnames(dst.frame) <- c('V1', 'V2', 'DOY', 'Dst')
res <- as.POSIXlt(paste(dst.frame$V1, dst.frame$V2), "%d/%m/%y %H:%M:%OS")
timer <- res$mday + res$hour/24 + res$min/(24*60) + res$sec/(24*3600)## TO SOLVE day of the month!
...
plot(dst3$time, dst3$Dst, type='l', col='red', xlab='time [days in September, 2017]', ylab='Dst [nT]')

dst4 <- dst[dst$time < 29,]
dst4 <- dst4[dst4$time >= 26,]
plot(dst4$time, dst4$Dst, type='l', col='red', xlab='time [days in September, 2017]', ylab='Dst [nT]')
par(mfrow=c(1,1))
...
par(mfrow=c(2,2))
plot(envi$time, envi$TEC, type='l', 
     col='red', xlab='time [DOY]', ylab = 'TEC [TECU]')
plot(envi$time, envi$Bx, type='l', 
     col='red', xlab='time [DOY]',ylab='Bx [nT]')
plot(envi$time, envi$By, type='l',
     col='red', xlab='time [DOY]',ylab='By [nT]')
plot(envi$time, envi$Bz, type='l',
     col='red', xlab='time [DOY]',ylab='Bz [nT]')
par(mfrow=c(1,1))

par(mfrow=c(2,2))
plot(density(envi$TEC), main = 'TEC', xlab = 'TEC [TECU]', 
     ylab = 'density')
plot(density(envi$Bx), main = 'Bx', xlab = 'Bx [nT]', ylab = 'density')
plot(density(envi$By), main = 'By', xlab = 'By [nT]', ylab = 'density')
plot(density(envi$Bz), main = 'Bz', xlab = 'Bz [nT]', ylab = 'density')
par(mfrow=c(1,1))
...
library(dtw);
alignment<-dtw(envi$Bz, envi$TEC,keep=TRUE);
...
plot(
  dtw(envi$Bz, envi$TEC,keep=TRUE,
      step=rabinerJuangStepPattern(6,"c")),
  type="twoway",offset=-2);

for(i in 1:10){
  p.reti <- grangertest(data20$PM10,data20$retail, order=i)
  print(p.reti)
  p.ret[i,1] <- i
  p.ret[i,2] <- p.reti$`Pr(>F)`[2]
  
  p.trani <- grangertest(data20$PM10,data20$transit, order=i)
  print(p.trani)
  p.tran[i,1] <- i
  p.tran[i,2] <- p.trani$`Pr(>F)`[2]
  
  p.groci <- grangertest(data20$PM10,data20$grocery, order=i)
  print(p.groci)
  p.groc[i,1] <- i
  p.groc[i,2] <- p.groci$`Pr(>F)`[2]
  
  p.worki <- grangertest(data20$PM10,data20$work, order=i)
  print(p.worki)
  p.work[i,1] <- i
  p.work[i,2] <- p.worki$`Pr(>F)`[2]
  
  p.resi <- grangertest(data20$PM10,data20$residential, order=i)
  print(p.resi)
  p.res[i,1] <- i
  p.res[i,2] <- p.resi$`Pr(>F)`[2]
}
...
par(mfrow=c(2,2))
plot(density(envi$TEC), main = 'TEC', xlab = 'TEC [TECU]', ylab = 'density')
plot(density(envi$Bx), main = 'Bx', xlab = 'Bx [nT]', ylab = 'density')
plot(density(envi$By), main = 'By', xlab = 'By [nT]', ylab = 'density')
plot(density(envi$Bz), main = 'Bz', xlab = 'Bz [nT]', ylab = 'density')
par(mfrow=c(1,1))
...
boxplot(envi$Bz,  main = 'Bz', xlab = 'Bz [nT]', ylab = 'number of occurancies')
        #yaxt="n")
#axis(2, at=c(0:envi$Bz/1000), labels=sprintf("%1.f", envi$Bz/1000))
#axis(side = 1, at = seq(-30, -29, 10), labels = seq(-30, -29, 10))
par(mfrow=c(1,1))
...
descdist(iono1$TEC, boot=1000)
descdist(iono2$TEC, boot=1000)
descdist(iono3$TEC, boot=1000)
descdist(iono4$TEC, boot=1000)

descdist(geom1$Bx, boot=1000)
descdist(geom1$By, boot=1000)
descdist(geom1$Bz, boot=1000)

descdist(geom2$Bx, boot=1000)
descdist(geom2$By, boot=1000)
descdist(geom2$Bz, boot=1000)

descdist(geom3$Bx, boot=1000)
descdist(geom3$By, boot=1000)
descdist(geom3$Bz, boot=1000)

descdist(geom4$Bx, boot=1000)
descdist(geom4$By, boot=1000)
descdist(geom4$Bz, boot=1000)
...
print(paste('Processing time = ',lm.proc,' s.', sep=''))
print('Linear regression model')
print(summary(fit.lm))
lm_pred <- predict(fit.lm, testing)
postResample(pred=lm_pred, obs=testing$TEC)
plot(testing$TEC, lm_pred)
adjr2 <- summary(fit.lm)$adj.r.squared

#CART1 **
set.seed(13)
ptm <- Sys.time()
fit.cart1 <- train(TEC~., data=dataTrain, method="rpart1SE", 
                   trControl=trainControl)
cart1.proc <- Sys.time() - ptm
print(paste('Processing time = ',cart1.proc,' s.', sep=''))
cart1_pred <- predict(fit.cart1, testing)
cart1.pr<-postResample(pred=cart1_pred, obs=testing$TEC)
cart1 <- get_best_result(fit.cart1)
plot(testing$TEC, cart1_pred)
print('CART1')
print(cart1.pr)
#plot_qq(cart1_pred-testing$TEC)
...
set.seed(13)
ptm <- Sys.time()
fit.gamb <- train(TEC~., data=dataTrain, method="gamboost", 
                   trControl=trainControl)
gamb.proc <- Sys.time() - ptm
print(paste('Processing time = ',gamb.proc,' s.', sep=''))
gamb_pred <- predict(fit.gamb, testing)
gamb.pr<-postResample(pred=gamb_pred, obs=testing$TEC)
gamb <- get_best_result(fit.gamb)
plot(testing$TEC, gamb_pred, xlab='observed TEC',
     ylab='gamb-predicted TEC')
...
#       cart1.pr[1], kl_RMSE_testing)
#M <- c("BCART","SGB","CART1", "K")

H <- c(bcart.pr[1],sgb.pr[1], gamb.pr[1],
       kl_RMSE_testing)
M <- c("BC","SGB", "GAMB", "K")
...
# Plot the bar chart 
barplot(H,names.arg=M,xlab="method",ylab="RMS  [TECU]",col="gray",
        main="RMSE",border="red", cex.lab=1.4)

# Create the data for the adjR2 chart
#H <- c(bcart.pr[2]*100, sgb.pr[2]*100,
#       cart1.pr[2]*100, kl_adjR2_testing*100)
H <- c(bcart.pr[2]*100, sgb.pr[2]*100, gamb.pr[2]*100,
...
# Create the data for the adjR2 chart
H <- as.vector(as.numeric(c(cart1.proc,bcart.proc,sgb.proc,gamb.proc)))
M <- c("PPR","BCART","SGB","GAMB")

# Give the chart file a name
#png(file = "model development time.png")

# Plot the bar chart 
barplot(H,names.arg=M,xlab="method",ylab="time elapsed [s]",col="gray",
...
plot(testing$TEC, gamb_pred, main='GAMB',
     xlab = 'observed', ylab = 'predicted', cex.lab = 2)
a<- c(0,50)
b<-c(0,50)
dabl <- as.data.frame(cbind(a,b))
...
t.test(envi1$TEC, envi2$TEC)
t.test(envi1$TEC, envi3$TEC)
t.test(envi1$TEC, envi4$TEC)
t.test(envi2$TEC, envi3$TEC)
t.test(envi2$TEC, envi4$TEC)
t.test(envi3$TEC, envi4$TEC)

var.test(envi1$TEC, envi2$TEC)
var.test(envi1$TEC, envi3$TEC)
var.test(envi1$TEC, envi4$TEC)
var.test(envi2$TEC, envi3$TEC)
var.test(envi2$TEC, envi4$TEC)
var.test(envi3$TEC, envi4$TEC)

ks.test(envi1$TEC, envi2$TEC)
ks.test(envi1$TEC, envi3$TEC)
ks.test(envi1$TEC, envi4$TEC)
ks.test(envi2$TEC, envi3$TEC)
ks.test(envi2$TEC, envi4$TEC)
ks.test(envi3$TEC, envi4$TEC)

res.sgb <- sgb_pred - testing$TEC
boxplot(res.sgb)
ks.test(res.sgb,'dnorm')
plot(density(res.sgb))
