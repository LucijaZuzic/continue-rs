...
sink(paste('TEC_',limited,'/correlation_output_',limited,'.txt',sep=''))
...
# ==============================================================================
# ADDED ANALYSIS: Correlation Heatmap and KS Tests (After filtering TEC >= 150)
# ==============================================================================

# Create a combined data frame of the filtered values
df_analysis<-data.frame(TEC=iono1$TEC,Bx=geom1$Bx,By=geom1$By,Bz=geom1$Bz)

# Strip any unexpected NAs before statistical tests
df_analysis<-na.omit(df_analysis)

# 1. Print Correlation Matrix to the sink text file
cat('\n\n--- PEARSON CORRELATION MATRIX (TEC,Bx,By,Bz) ---\n')
cor_matrix<-cor(df_analysis)
print(cor_matrix)

# 2. Save the Correlation Heatmap image using DataExplorer
pdf(file=paste('TEC_',limited,'/correlation_heatmap_TEC_Bxyz_',limited,'.pdf',sep=''))
# Plot_correlation automatically generates a ggplot heatmap of continuous features
p<-plot_correlation(df_analysis,type='continuous',title=paste('Correlation Heatmap (TEC < ',limited,')',sep=''))
dev.off()

# 3. Perform Kolmogorov-Smirnov test for normality on each variable individually
cat('\n\n--- KOLMOGOROV-SMIRNOV NORMALITY TESTS ---\n')
for (var_name in names(df_analysis)) {
  var_data<-df_analysis[[var_name]]
  # Standardize and test against standard normal (or directly use sample mean/sd)
  # Ties generate warnings in KS test,which is fine since options(warn=-1) handles it 
